...
//* Number 클래스만 타입 매개변수로 사용가능하도록 제한
...
//		d3.setT(new A());
		...
		//* 타입 지정을 생략한 경우
		//제네릭 타입을 제한한 A 클래스를기본으로 타입지정하여 생성
		D d5 = new D();		//D<A> d5 = new D<A>();
		d5.setT(new B());
		d5.setT(new B());
		
		//E <T extends Number>
		//* Number 클래스로 제한해주어서 숫자타입인 클래스들만 타입지정 가능
		E<Integer> e1 = new E<Integer>();
		E<Double> e2 = new E<Double>();
		E<Byte> e3 = new E<Byte>();
		//(에러 : 숫자타입 X)E<Boolean> e4 = new E<Boolean>();
		...
		//그냥 쓴 정수는 (124) int 타입으로 인식 4byte
		//e3 객체에서 지정한 타입은 Byte (1byte) 이기 때문에 강제 형변환 필요
		e3.setT((byte)127);
